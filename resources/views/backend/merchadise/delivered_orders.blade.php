
@extends('backend.adminmaster')
@section('title','Delivered Orders')
@section('content')
<div class="container">
    @if(Session::has('success'))
        <div class="alert alert-success">
            {{Session::get('success')}}
        </div>
    @endif
    {{-- Show all orders --}}
    <div class="container">
        <div class="row shadow mb-5 bg-black rounded">
            <div class="col-lg-12">
              <div class="panel-heading mt-5" style="text-align: center;"> 
                  <h3 class="mb-2 panel-title">All Orders Placed By Clients</h3> 
              </div>
              <table id="adminorderstable" class="table table-striped table-bordered nowrap" style="width:100%; border:2px solid black;">
                  <thead>
                      <tr>
                          <th>Id</th>
                          <th>Phone</th>
                          <th>Town</th>
                          <th>Order Status</th>
                          <th>Tracking Id</th>
                          <th>Action</th>
                      </tr>
                  </thead>
                  <tbody>
                  </tbody>
              </table>
            </div>
        </div>
    </div>
    <!-- end col -->
</div>
@endsection

@section('adminordersscript')
    
    <script>

        // show all orders in a datatable
        var adminorderstable = $('#adminorderstable').DataTable({
            processing:true,
            serverside:true,
            responsive:true,

            ajax:"{{ route('users.deliveredorders') }}",
            columns: [
                { data: 'id' },
                { data: 'phone' },
                { data: 'town' },
                { data: 'order_status' },
                { data: 'tracking_id' },
                { data: 'action',name:'action',orderable:false,searchable:false },
            ],
        });

        // show modal to change order status
        $(document).on('click','#orderdelivered',function(e){
            e.preventDefault();
            var order_id=$(this).val();
            var url = '{{ route("user.order", ":id") }}';
                    url = url.replace(':id', order_id);
            $('#pickordermodal').modal('show');
            
            $.ajax({
                type:"GET",
                url:url,
                processData: false,
                contentType: false,
                success:function(response)
                {
                    console.log(response);
                    if (response.status==200)
                    {
                        $('#recipient_order_id').val(response.orderdata.id);
                        $('#picked_order_tracking_id').val(response.orderdata.tracking_id);
                        $('.orderstatus').append('<option disabled="true">In Transit</option>');
                        $('.orderstatus').append('<option disabled="true">Delivered</option>');
                        $('.orderstatus').append('<option value="Picked">Picked</option>');
                    }
                }
            })
        })

         // update customer order status to shipping
         $(document).on('submit','#pickorderform',function(e){
            e.preventDefault();

            var orderid=$('#recipient_order_id').val();
            var url = '{{ route("update_order.picking", ":id") }}';
                url = url.replace(':id', orderid);

            $.ajax({
                url:url,
                type:"POST",
                data:$("#pickorderform").serialize(),
                success:function(response){
                    console.log(response);
                    
                    if (response.status==200)
                    {
                        $('#pickordermodal').modal('hide');
                        adminorderstable.ajax.reload(null,false);
                        alertify.set('notifier','position', 'top-right');
                        alertify.success(response.message);
                        $('#recipient_order_id').val('');
                        $('#picked_order_tracking_id').val('');
                    }
                }
                ,error: function(error)
                {
                    console.error(error)
                }
            });
        })
    </script>
@stop

